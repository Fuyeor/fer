...
Document formatting is available through `textDocument/formatting`. The formatter validates the source, normalizes code-line indentation to the client tab/space preference, and applies safe canonical spacing around assignments, arrows, arithmetic/comparison operators, commas, colons, braces, and quantifier/`not` parentheses. Ordinary calls, grouping, indexing, member access, import paths, and unary minus retain their grammar-required tight spacing. It returns one full-document edit only when the source changes, and preserves comments, string and interpolation spelling, regex bodies, line endings, and unknown trivia. It refuses to rewrite lexical errors, parser errors, or unbalanced braces; separator reflow and line breaking are intentionally outside the current formatter scope.
